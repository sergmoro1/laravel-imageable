...
В этом же или более подходящем шаблоне необходимо вставить регистрацию опций загрузки.
```
    <x-slot name="scripts">
      <script>var uploadOptions = <?= $post->uploadOptions() ?>;</script>
    </x-slot>
```
Например, для модели `Post` компонет `<x-imageable-upload :model="$post"/>` вставлен в представлении
`views/post/form.blade.php`, там же может быть выполнена регистрация опций.

Если на страницу уже подключены необходимые `css` и `js` файлыи, а это возможно, если вы уже подключали `Imageable` для другой модели, то можно загружать изображения.
...
```
Очевидно можно вносить коррективы в определение классов в файле `resources/css/imageable.upload.css`.
...
Предполагается, что можно загрузить больше одного файла для конкретной модели, 
поэтому можно добавить плагин [`Sortable`](https://github.com/SortableJS/Sortable) сортировки порядка изображений. 
...
```
<x-imageable-upload :model="$model"/>
```
может быть вставлен в любом месте представления, не обязательно внутри тега `form`.
